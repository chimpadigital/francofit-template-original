<?php function thb_clients( $atts, $content = null ) {
	...
	$image = wpb_getImageBySize( array( 'attach_id' => $image, 'thumb_size' => 'full', 'class' => $retina ) );
	...
	<div class="<?php echo esc_attr(implode(' ', $el_class)); ?>" style="border-color: <?php echo esc_attr($thb_border_color); ?>">
		<?php if (in_array($thb_style, array('style4'))) { ?>
		  ...
		  <?php if (in_array($thb_style, array('style4'))) { ?>
		    ...
		    <?php if($link_to) { ?><a href="<?php echo esc_url($link_to); ?>" target="<?php echo esc_attr($a_target); ?>" class="button"><?php esc_html_e('Learn More', 'revolution' ); ?></a><?php } ?>
		  <?php } ?>
